...
export async function precrypt_file(input_path: string, file_key: umbral.SecretKey, output_path: string, threads: number, memory_size: number): Promise<void> {
...
export function recrypt_keys(recryption_keys: RecryptionKeys, receiver_public: umbral.PublicKey): DecryptionKeys {
   ...
   const translation_key = umbral.generateKFrags(
      ...
      false,
      false
   ) as unknown as [umbral.VerifiedKeyFrag];
